...
def compute_levels(candles: dict, strike_step: int) -> pd.DataFrame:
    ...
    ce_up1..4, ce_dn1..4, pe_up1..4, pe_dn1..4, bep_center
    ...
    bep_center = avg(ATM CE close, ATM PE close) -- the single shared "BEP
    STRIKE" line (n=0), used as the Reversal Entry target when a DN1
    reversal fires (there is no DN0 to step down to).
    """
    ...
    merged["bep_center"] = (merged["ce_close"] + merged["pe_close"]) / 2
    ...
    for n in OFFSETS[:-1]:  # UP1..UP4 -> target UP(n+1); UP5 has no UP6 above it
        if _crossed_up(prev_row[f"{side}_close"], prev_row[f"{side}_up{n}"],
            ...
            if row[f"{other}_close"] < row[f"{other}_dn{n}"]:
                ...
                target = row[f"{side}_up{n + 1}"]
                # UP(n+1) should always sit above UP(n) -- but a deep OTM leg can go
                # untraded for a whole bar, leaving a stale close that prints BELOW
                # the level just crossed. Don't open a trade off a broken target.
                if pd.isna(target) or target <= entry_price:
                    return None
                return {
                    "time": row["timestamp"], "side": tag,
                    "line": f"{side.upper()}-UP{n}", "entry": entry_price,
...
def _try_open_reversal(side, other, tag, prev_row, row, sl_buffer_pct):
    ...
    the "dipped below DNn, came back up, closed above DNn again" pattern.
    Confirmed by the OTHER leg sitting below ITS UPn at the matching level n
    ...
    for n in OFFSETS:  # DN1..DN5 -- every level can trigger a reversal
        if _crossed_up(prev_row[f"{side}_close"], prev_row[f"{side}_dn{n}"],
            ...
            if row[f"{other}_close"] < row[f"{other}_up{n}"]:
                entry_price = row[f"{side}_close"]
                target = row["bep_center"] if n == 1 else row[f"{side}_dn{n - 1}"]
                # target should always sit above the DNn line just crossed -- same
                # stale-leg protection as _try_open, don't open off a broken target.
                if pd.isna(target) or target <= entry_price:
                    return None
                return {
                    "time": row["timestamp"], "side": tag,
                    "line": f"{side.upper()}-DN{n} (Reversal)", "entry": entry_price,
                    "target": target, "target_n": n - 1,
                    "sl": _sl_from_low(row[f"{side}_low"], sl_buffer_pct), "exit": None,
                    "result": "In Trade", "pnl_points": None,
                }
...
def build_journal(df: pd.DataFrame, max_rows: int, sl_buffer_pct: float = 0.0,
    ...
    PE-Trend, PE-Reversal. Each slot holds at most one open trade and is
    otherwise fully independent of the others -- e.g. a CE Trend trade being
    open does NOT block a CE Reversal trade from opening at the same time,
    and vice versa.

    Trend entry    -> <side> closes fresh ABOVE its own UPn, confirmed by the
                       other leg closing BELOW its own DNn (same level n).
                       Target = the next UP level, UP(n+1).
    Reversal entry -> <side> closes fresh back ABOVE its own DNn (dipped to/
                       below DNn, then recovered and closed above it again),
                       confirmed by the other leg closing BELOW its own UPn
                       (same level n). Target = the next level toward the
    ...
    for _, row in df.iterrows():
        if prev is not None:
            ...
            if open_ce_trend is None:
                ...
                if row["ce_low"] <= open_ce_trend["sl"]:
                    _close_trade(open_ce_trend, open_ce_trend["sl"], "SL Hit")
                    open_ce_trend = None
                elif not pd.isna(open_ce_trend["target"]) and row["ce_high"] >= open_ce_trend["target"]:
                    _close_trade(open_ce_trend, open_ce_trend["target"], "Target Hit")
                    open_ce_trend = None
            ...
            if open_ce_rev is None:
                ...
                if row["ce_low"] <= open_ce_rev["sl"]:
                    _close_trade(open_ce_rev, open_ce_rev["sl"], "SL Hit")
                    open_ce_rev = None
                elif not pd.isna(open_ce_rev["target"]) and row["ce_high"] >= open_ce_rev["target"]:
                    _close_trade(open_ce_rev, open_ce_rev["target"], "Target Hit")
                    open_ce_rev = None
            ...
            if open_pe_trend is None:
                ...
                if row["pe_low"] <= open_pe_trend["sl"]:
                    _close_trade(open_pe_trend, open_pe_trend["sl"], "SL Hit")
                    open_pe_trend = None
                elif not pd.isna(open_pe_trend["target"]) and row["pe_high"] >= open_pe_trend["target"]:
                    _close_trade(open_pe_trend, open_pe_trend["target"], "Target Hit")
                    open_pe_trend = None
            ...
            if open_pe_rev is None:
                ...
                if row["pe_low"] <= open_pe_rev["sl"]:
                    _close_trade(open_pe_rev, open_pe_rev["sl"], "SL Hit")
                    open_pe_rev = None
                elif not pd.isna(open_pe_rev["target"]) and row["pe_high"] >= open_pe_rev["target"]:
                    _close_trade(open_pe_rev, open_pe_rev["target"], "Target Hit")
                    open_pe_rev = None
